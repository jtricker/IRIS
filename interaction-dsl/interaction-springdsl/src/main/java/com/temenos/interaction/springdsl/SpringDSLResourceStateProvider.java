...
public class SpringDSLResourceStateProvider implements ResourceStateProvider, DynamicRegistrationResourceStateProvider {
    ...
    /*
     * Use a Boolean, rather than a bool, so access can be synchronized.
     */
     ...
     */
    protected Map<String, Set<String>> resourceStatesByPath = new ConcurrentHashMap<String, Set<String>>();
    /**
     * Map of request to state names
     */
    protected Map<String, String> resourceStatesByRequest = new ConcurrentHashMap<String, String>();
    /**
     * Map of resource methods where state name is the key
     */
    protected Map<String, Set<String>> resourceMethodsByState = new ConcurrentHashMap<String, Set<String>>();
    /**
     * Map to a resource path where the state name is the key
     */
    protected Map<String, String> resourcePathsByState = new ConcurrentHashMap<String, String>();
    ...
    protected void initialise() {
        // In the common case, where we are initialized, return without the
        // overhead of taking a lock.
        if (initialised)
            return;

        // We may have to initialize. Must be atomic so take a lock..
        synchronized (initialised) {
            if (initialised)
                // Somebody else initialized first.
                return;
    ...
    /**
     * This changes multiple maps. Must be atomic.
     * 
     * @param stateObj
     * @param binding
     */
    protected synchronized void storeState(Object stateObj, String binding) {
        synchronized (resources) {
            ...
            for (String method : methodSet) {
                ...
                if (found != null) {
                    logger.error("Multiple states bound to the same request [" + request + "], overriding [" + found
                            + "] with [" + stateName + "]");
                }
     ...
     * This changes multiple maps. Must be atomic.
     */
    public synchronized void addState(String stateObj, Properties properties) {
        if (initialised) {
            String stateName = stateObj.toString();
    ...
    public void unload(String name) {
        // Someone else may be loading resources. So be atomic.
        synchronized (resources) {
    ...
    public ResourceState getResourceState(String resourceStateName) {
        ...
        try {
            if (resourceStateName != null) {
                // First try access without synchronization. In the common case,
                // where resource is already loaded, this should succeed without
                // the overhead of taking a lock.

                // Try to retrieve the resource state
                result = resources.get(resourceStateName);

                if (result == null) {
                    // Resource state has not already been loaded. Attempt
                    // to load it. This may be called by multiple, lazy loading,
                    // connections. Take a lock and do it atomically.
                    synchronized (resources) {
                        // Try again
                        result = resources.get(resourceStateName);

                        if (result == null) {
                            // Nobody else loaded it first.
     ...
     * resource could be: this tracks all of them so that they can be logged if
    ...
    private class ResourceStateLoad {
        ...
        /**
         * Define a resource state to load. Must call load() to actually load
         * it.
         */
         ...
         * 
         * May be called by multiple, lazy loading, sessions. So must be atomic.
         */
